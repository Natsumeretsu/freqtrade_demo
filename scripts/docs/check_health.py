...
运行方式（本仓库推荐）：
  uv run python "scripts/docs/check_health.py"

可选：
  uv run python "scripts/docs/check_health.py" --include-reference-secrets
  uv run python "scripts/docs/check_health.py" --check-config-examples
"""
