...
## 0) 当前环境状态（浏览器与抓取）
- 若缺少 Chrome 导致 `playwright_mcp` / `chrome_devtools_mcp` 不可用：运行 `./scripts/tools/fix_chrome_for_mcp.ps1`（无需管理员权限；等价手动步骤见 `docs/setup/codex_mcp_sync.md`）。
- 修复后：遇到需要 JS 的页面优先用 `playwright_mcp` 抓取；失败再登记为 blocked。



## 1) 文档结构（已迁移）
- 原 `other_doc/` 已迁移为 `docs/`，并按职责分层：
  - `docs/design/`：策略赛道与策略设计文档（含“策略类唯一基底”）
  - `docs/guidelines/`：工程规范（例如回测输出标准）
  - `docs/setup/`：工具/环境同步（Claude/Codex 等）
  - `docs/knowledge/`：外部来源登记与可持续知识沉淀

## 2) 策略文档规则（强制）
- “一类策略（一个 `IStrategy` 子类）→ 一份唯一基底文档”。
- 唯一入口索引：`docs/freqai_core_strategy_guide.md`。
- 策略细节只写在该策略对应的基底文档，索引文件只挂链接，避免多源冲突。

## 3) 回测输出标准（强制）
- 规范：`docs/guidelines/backtest_reporting_standard.md`。
...
## 4) 项目知识沉淀机制（主入口）
- 知识索引：`docs/knowledge/index.md`。
- 来源登记（权威清单）：`docs/knowledge/source_registry.md`（URL/抓取方式/状态/可复用要点/落地映射）。
...
4. 将已验证要点回灌到对应设计文档/策略基底文档，并在 `docs/freqai_core_strategy_guide.md` 挂入口。

## 5) 已抓取来源（按类别，详情以登记表为准）
> 完整列表与抓取状态以 `docs/knowledge/source_registry.md` 为准。
...
- 本仓库策略文档已明确：入场/出场应对齐同一个 `label_period_candles` 与同一套 barrier 口径（见 `docs/design/freqai_cta_trend_v3.md`）。

### 6.3 样本不平衡：先解决“标签稀缺 → 过度保守”
- LightGBM 的不平衡处理二选一：`is_unbalance` 或 `scale_pos_weight`（S-505）。
- 文档提醒：不平衡权重会影响概率估计质量；因此在策略层应把概率更多当作“排序/优势”而非绝对校准值，并用 `enter_prob + prob_margin` 结构降低抖动（见 `docs/design/freqai_cta_trend_v3.md`）。
...
## 7) 已回灌位置（入口一致）
- 赛道总览：`docs/design/crypto_futures_strategy_options.md`。
- 策略唯一入口索引：`docs/freqai_core_strategy_guide.md`。
- 知识索引/来源登记：`docs/knowledge/index.md` / `docs/knowledge/source_registry.md`。
