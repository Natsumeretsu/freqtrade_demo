...
这些产物属于**可重建缓存/证据层**，默认不作为“权威结论”；权威结论应写入 `docs/` 与 `.serena/memories/`。
...
1) 在 `docs/knowledge/source_registry.md` 登记 URL（分配 `S-xxx`）
2) 批量采集并落盘（静态优先）：
...
vbrain 控制平面见：`docs/tools/vbrain/README.md`。
