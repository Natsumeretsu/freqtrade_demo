...
- 采集层控制平面：`docs/tools/vharvest/README.md`
...
3. Backfill：回灌到权威知识层（`docs/` + `.serena/memories/`）
4. Memory：把“套路/决策/坑点”写入 `in_memoria`
5. Index：同步索引（按需 ingest `docs` 与外部材料）
...
- 评估 Local RAG 嵌入模型（回归集 + hit@k/MRR；默认不破坏主 DB）：
  - `python -X utf8 scripts/tools/vbrain.py eval-embeddings -- --models "Xenova/multilingual-e5-large,Xenova/multilingual-e5-base,Xenova/bge-m3"`
  - 选型与速度基准结论（可跨设备复用）：`docs/setup/vibe_brain_workflow.md`（4.1.1）
...
- `docs/tools/vbrain/manifest.json`：可机器读取的清单（组件、路径、脚本入口）
- `scripts/tools/vbrain_mcp_server.py`：婴儿版 MCP Server（对外暴露高层工具，内部复用 `scripts/tools/vbrain.py`）
- `docs/tools/vbrain/mcp_blueprint.md`：后续演进蓝图（例如：打包为可分发的 MCP / 增强进度通知等）
