...
- 来源：https://www.freqtrade.io/en/stable/advanced-setup/
- 离线保存时间：Mon Jan 05 2026 11:42:28 GMT+0800 (中国标准时间)
...
For more information regarding usage of the sqlite databases, for example to manually enter or remove trades, please refer to the [SQL Cheatsheet](https://www.freqtrade.io/en/stable/sql_cheatsheet/).
...
Usage:
`... --db-url postgresql+psycopg://:@localhost:5432/`
...
Usage:
`... --db-url mysql+pymysql://:@localhost:3306/`
...
We recommend to extract the logging configuration from your main freqtrade configuration file, and provide it to your bot via [multiple configuration files](https://www.freqtrade.io/en/stable/configuration/#multiple-configuration-files) functionality. This will avoid unnecessary code duplication.
...
- `"address": ":514"` -- log to remote syslog at IP address and port 514. This may be used on Windows for remote logging to an external syslog server.

Deprecated - configure syslog via command line
`--logfile syslog:` -- send log messages to `syslog` service using the `` as the syslog address.
...
- `--logfile syslog::514` -- log to remote syslog at IP address and port 514. This may be used on Windows for remote logging to an external syslog server.
