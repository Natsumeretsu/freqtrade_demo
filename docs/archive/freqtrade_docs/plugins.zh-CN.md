...
- 来源：https://www.freqtrade.io/en/stable/plugins/
- 离线保存时间：Mon Jan 05 2026 11:36:33 GMT+0800 (中国标准时间)
...
Pairlist configurations can be quite tricky to get right. Best use freqUI in [webserver mode](https://www.freqtrade.io/en/stable/freq-ui/#webserver-mode) or the [`test-pairlist`](https://www.freqtrade.io/en/stable/utils/#test-pairlist) utility sub-command to test your Pairlist configuration quickly.
...
PercentChange=(CurrentClose−PreviousClosePreviousClose)∗100
...
With `ProducerPairList`, you can reuse the pairlist from a [Producer](https://www.freqtrade.io/en/stable/producer-consumer/) without explicitly defining the pairlist on each consumer.

[Consumer mode](https://www.freqtrade.io/en/stable/producer-consumer/) is required for this pairlist to work.
...
Namely, pairs are blacklisted if a variance of one percent or more in the stop price would be caused by precision rounding on the exchange, i.e. `rounded(stop_price)
...
| --- | --- |
| method | Protection name to use. Datatype: String, selected from available Protections |
