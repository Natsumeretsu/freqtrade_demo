...
> 推荐优先使用已修复并对齐为分类器的 `FreqaiMetaLabelV2`（见：`docs/archive/design/freqai_meta_label_v2.md`）。

- 策略代码：`01_freqtrade/strategies_archive/FreqaiMetaLabel.py`
- 推荐配置：`04_shared/configs/archive/freqai/meta_label_1h_v1.json`
...
> 参考实现：`01_freqtrade/strategies_archive/FreqaiMetaLabelV2.py`
...
./scripts/ft.ps1 backtesting `
  --config "04_shared/configs/archive/freqai/meta_label_1h_v1.json" `
...
- 直接迁移到 `FreqaiMetaLabelV2`：`docs/archive/design/freqai_meta_label_v2.md`。\n- 若你坚持保留 v1，请先完成第 3 节的三项修复，并重新做 walk-forward 评估与成本敏感性分析。  
