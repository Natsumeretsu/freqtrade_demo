...
来源：`docs/knowledge/source_registry.md` 的 S-303 / S-304 / S-305 / S-306。
...
- 使用 `FreqaiVolatilityGridV1`（波动率门控 + maker 挂单 + 均值回归退出）作为网格风格起点：  
  - 策略：`01_freqtrade/strategies_archive/FreqaiVolatilityGridV1.py`  
  - 配置：`04_shared/configs/archive/freqai/volatility_grid_btc_1h_v1.json`
...
- 下一步建议用 `scripts/archive/freqai/param_sweep.py` 重点扫：
...
- 震荡/均值回归/网格风格：优先 `FreqaiVolatilityGridV1`  
  - 文档：`docs/archive/design/freqai_volatility_grid_v1.md`
- 趋势/CTA（单币对 + 多空）：`FreqaiCTATrendV3`（当前回测表现较弱，作为研究基线）  
  - 文档：`docs/archive/design/freqai_cta_trend_v3.md`

---

## 5) 已验证来源（MCP 抓取摘要）

完整来源登记与抓取状态见：`docs/knowledge/source_registry.md`。
