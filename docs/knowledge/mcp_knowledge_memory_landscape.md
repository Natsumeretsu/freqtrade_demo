...
1) **来源层（raw）**：URL 登记在 `docs/knowledge/source_registry.md`。  
2) **采集层（feed/cache）**：用 `vharvest` 采集并落盘为本地 feed（默认在 `.vibe/knowledge/sources/`）。  
3) **提炼层（asset）**：把关键结论写回 `docs/`（可追溯、可索引、可复用）。  
4) **记忆层（brain）**：把决策/套路用 `in_memoria.contribute_insights` 固化为可检索的 insight。  
5) **检索层（accelerator）**：`local_rag` 索引 `docs/`，用于高质量召回与关键词精确命中。
