...
- 回测/超参/绘图产物（`01_freqtrade/backtest_results/`、`01_freqtrade/hyperopt_results/`、`01_freqtrade/plot/`）
- 训练产物与模型缓存（`01_freqtrade/models/`、`02_qlib_research/models/` 等）
...
- `01_freqtrade/config.json`（运行配置：允许同步，但必须保持 key/secret/password/token 为空；密钥放到 `.env` 或 `01_freqtrade/config-private.json`）
- `in-memoria.db`（主脑 DB：跨设备一致性更重要，体积可控）

### 2.2 代码与配置

- `01_freqtrade/strategies/`：策略实现（Python）
- `04_shared/configs/`：可提交、可复制的配置模板（脱敏，不含密钥）
- `04_shared/config/`：Qlib 研究/集成的 YAML 配置（可提交）
- `scripts/`：统一命令入口与工具链（`scripts/lib/`、`scripts/tools/` 必须同步）
- `01_freqtrade/hyperopts/`：Hyperopt loss / 目标函数（源码）
- `03_integration/`：集成桥接代码（`trading_system/` 等）
- `02_qlib_research/notebooks/`：可复现的研究/分析笔记（如确有价值）

### 2.3 文档与知识层

- `docs/`：权威知识库（设计/知识/维护规则）
- `.serena/project.yml`、`.serena/memories/`：流程与记忆（可读、可维护）
- `docs/tools/vbrain/`、`docs/tools/vharvest/`：控制平面文档（清单/蓝图/入口说明）
- `docs/archive/freqtrade_docs/`、`docs/archive/freqtrade_book/`：离线文档（注意：`docs/archive/freqtrade_docs/raw_html/` 默认不提交）
...
- 不应：`docs/` 与 `.serena/memories/` 的文本结论不应出现差异（受 Git 约束）。
...
- `01_freqtrade/logs/`、`01_freqtrade/backtest_results/`、`01_freqtrade/hyperopt_results/`、`01_freqtrade/plot/`
- `01_freqtrade/models/`、`02_qlib_research/models/`（训练产物目录）
- `artifacts/`
- `temp/`（草稿/临时研究文件，建议把“成熟结论”迁入 `docs/`）
- `in-memoria-vectors.db/`、`lancedb/`（本地索引/向量缓存）

---

## 5) 市场数据 `01_freqtrade/data/` 的策略（重要）

现状：`01_freqtrade/data/` 往往体积大、更新频繁，会导致：
...
- 默认不使用 Git 同步 `01_freqtrade/data/`（已在 `.gitignore` 忽略）。
- 需要数据时，通过脚本重新下载：
...
- 取消跟踪市场数据（推荐）：
  - `git rm --cached -r 01_freqtrade/data/`
- 取消跟踪 Claude 本地设置（推荐）：
