...
class TestTimingPoolV2_200(unittest.TestCase):
    def test_timing_pool_v2_200_size_and_support(self) -> None:
        names = render_factor_names(get_factor_templates("timing_pool_v2_200"), {})
    ...
    def test_timing_pool_koop_v2_200_size_and_contains_new_horizons(self) -> None:
        names = render_factor_names(get_factor_templates("timing_pool_koop_v2_200"), {})
        # 200（传统） + 10（Koopa-lite v2） = 210
        self.assertEqual(len(names), 210)

        for n in ["koop_pred_ret_h2", "koop_pred_ret_h8", "koop_pred_ret_h16"]:
