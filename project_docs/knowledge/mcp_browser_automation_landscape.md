...
每条来源（`S-xxx`）缓存目录：`.vibe/knowledge/sources/S-xxx/`（默认 gitignore，可重建）

- `markitdown.md` + `meta.json`（静态抓取）
- `playwright_snapshot.md` + `meta_playwright.json`（浏览器抓取）
- （建议新增）`playwright_dom.html`：渲染后 DOM（用于最大化保真）
- （建议新增）`playwright_network.json`：网络请求清单（用于复现/排障/后续抓 API）
...
下一步（落地文件入口）：
- 浏览器渲染采集入口：`scripts/tools/vharvest.py`（子命令：`fetch-playwright`）
- 来源登记：`project_docs/knowledge/source_registry.md`
- vibe brain 工作流：`project_docs/setup/vibe_brain_workflow.md`
