...
3. 给出与本仓库当前策略（`SmallAccountTrendFilteredV1`）的可结合点，并标注过拟合风险与验证方法。
...
相关来源登记（用户提供，待抓取）：见 `project_docs/knowledge/source_registry.md` 的 S-935~S-944。
...
- 可用 `custom_entry_price()` 把信号对应的挂单价下移到“Pin Bar 50%”附近（示例实现可参考 `strategies/FreqaiVolatilityGridV1.py`）。
...
## 5) 与 `SmallAccountTrendFilteredV1` 的低过拟合结合点（候选）
