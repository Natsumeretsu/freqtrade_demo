...
补充：
- 综合结论（局部帕累托前沿判断）见：`project_docs/reports/change_summary_2026-01-12.md`
- 注意：`artifacts/benchmarks/` 与 `backtest_results/` 默认不随 git 同步，本报告仅记录 RunId 与关键数字；跨设备请用 `-RunId` 复现。

---

## 1) 测试环境与口径

- 交易所与数据：OKX spot，本地 `data/okx`（BTC/USDT，4h）
- 成本假设（基准）：fee=0.0006
- 成本敏感性：fee=0.0012（2x）
...
对应文件：`strategies/SmallAccountTrendFilteredV1.py`
参数文件：`strategies/SmallAccountTrendFilteredV1.json`
...
uv run python -X utf8 "scripts/analysis/plot_equity.py" `
  --zip "backtest_results/<run_dir>/<backtest-result>.zip" `
...
uv run python -X utf8 "scripts/analysis/compare_backtest_zips.py" `
  --left-zip "backtest_results/<old_run_dir>/<backtest-result>.zip" `
  --right-zip "backtest_results/<new_run_dir>/<backtest-result>.zip" `
