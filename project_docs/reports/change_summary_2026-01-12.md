...
- 参数文件同步：新增/调整参数已写入 `strategies/SmallAccountTrendFilteredV1.json`，便于复现与后续回测。
...
注意：`artifacts/benchmarks/` 与 `backtest_results/` 默认不随 git 同步；跨设备请使用同名 `-RunId` 重新生成本地产物。
...
- EMA/MACD/Vegas：`project_docs/knowledge/ema_macd_vegas_playbook.md`
- K线/Pin Bar：`project_docs/knowledge/candlestick_pinbar_playbook.md`
- 流动性/微观结构：`project_docs/knowledge/crypto_liquidity_microstructure_playbook.md`

### 3.2 来源登记与抓取缓存

- 已将新增来源登记到：`project_docs/knowledge/source_registry.md`
