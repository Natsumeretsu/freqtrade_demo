...
./scripts/ft.ps1 backtesting `
  -c "configs/config_bear_rider.json" `
  --strategy "TheBearRider" `
  --timerange 20240101-20260101 `
  --export trades `
  --backtest-directory "backtest_results/fullcycle" `
  --no-color
```

运行完成后，会在 `backtest_results/fullcycle/.last_result.json` 写入最新 zip 名称。

### B. 生成逐交易对报表（HTML + CSV）

```powershell
$last = (Get-Content -Raw "backtest_results/fullcycle/.last_result.json" | ConvertFrom-Json).latest_backtest
uv run python "scripts/backtest_pair_report.py" `
  --zip ("backtest_results/fullcycle/" + $last) `
  --datadir "data/okx" `
  --timeframe "5m" `
...
默认输出：
- `plot/<Strategy>_per_pair_report_<YYYYMMDD>_<YYYYMMDD>.html`
- `plot/<Strategy>_per_pair_report_<YYYYMMDD>_<YYYYMMDD>.csv`
