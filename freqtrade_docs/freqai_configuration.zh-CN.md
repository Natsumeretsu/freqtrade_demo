...
- 来源：https://www.freqtrade.io/en/stable/freqai-configuration/
- 离线保存时间：Mon Jan 05 2026 11:40:22 GMT+0800 (中国标准时间)
...
FreqAI is configured through the typical [Freqtrade config file](https://www.freqtrade.io/en/stable/configuration/) and the standard [Freqtrade strategy](https://www.freqtrade.io/en/stable/strategy-customization/). Examples of FreqAI config and strategy files can be found in `config_examples/config_freqai.example.json` and `freqtrade/templates/FreqaiExampleStrategy.py`, respectively.

### Setting up the configuration file

Although there are plenty of additional parameters to choose from, as highlighted in the [parameter table](https://www.freqtrade.io/en/stable/freqai-parameter-table/#parameter-table), a FreqAI config must at minimum include the following parameters (the parameter values are only examples):
...
Notice how the `feature_engineering_*()` is where [features](https://www.freqtrade.io/en/stable/freqai-feature-engineering/#feature-engineering) are added. Meanwhile `set_freqai_targets()` adds the labels/targets. A full example strategy is available in `templates/FreqaiExampleStrategy.py`.
...
| df['DI_values'] | Dissimilarity Index (DI) values are proxies for the level of confidence FreqAI has in the prediction. A lower DI means the prediction is close to the training data, i.e., higher prediction confidence. See details about the DI here. Datatype: Float. |
...
- LightGBM: [https://lightgbm.readthedocs.io/en/v3.3.2/#](https://lightgbm.readthedocs.io/en/v3.3.2/#)

- XGBoost: [https://xgboost.readthedocs.io/en/stable/#](https://xgboost.readthedocs.io/en/stable/#)

- CatBoost: [https://catboost.ai/en/docs/](https://catboost.ai/en/docs/) (No longer actively supported since 2025.12)
...
- `BasePyTorchModel` - Implements the `train` method. all `BasePyTorch*` inherit it. responsible for general data preparation (e.g., data normalization) and calling the `fit` method. Sets `device` attribute used by children classes. Sets `model_type` attribute used by the parent class.

- `BasePyTorch*` - Implements the `predict` method. Here, the `*` represents a group of algorithms, such as classifiers or regressors. responsible for data preprocessing, predicting, and postprocessing if needed.

- `PyTorch*Classifier` / `PyTorch*Regressor` - implements the `fit` method. responsible for the main train flaw, where we initialize the trainer and model objects.
