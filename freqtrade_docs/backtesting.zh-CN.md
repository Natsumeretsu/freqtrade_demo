...
- 来源：https://www.freqtrade.io/en/stable/backtesting/
- 离线保存时间：Mon Jan 05 2026 11:38:47 GMT+0800 (中国标准时间)
...
Backtesting is also available in [webserver mode](https://www.freqtrade.io/en/stable/freq-ui/#backtesting), which allows you to run backtests via the web interface.
...
Backtesting will use the crypto-currencies (pairs) from your config file and load historical candle (OHLCV) data from `user_data/data/` by default.
...
For details on downloading, please refer to the [Data Downloading](https://www.freqtrade.io/en/stable/data-download/) section in the documentation.
...
Please read the [pairlists documentation](https://www.freqtrade.io/en/stable/plugins/#pairlists) for more information.
...
The exported trades can be used for [further analysis](#further-backtest-result-analysis) or can be used by the [plotting sub-command](https://www.freqtrade.io/en/stable/plotting/#plot-price-and-indicators) (`freqtrade plot-dataframe`) in the scripts directory.

#### Starting balance

Backtesting will require a starting balance, which can be provided as `--dry-run-wallet ` or `--starting-balance ` command line argument, or via `dry_run_wallet` configuration setting.
This amount must be higher than `stake_amount`, otherwise the bot will not be able to simulate any trade.

#### Dynamic stake amount

Backtesting supports [dynamic stake amount](https://www.freqtrade.io/en/stable/configuration/#dynamic-stake-amount) by configuring `stake_amount` as `"unlimited"`, which will split the starting balance into `max_open_trades` pieces.
...
You can then load the trades to perform further analysis as shown in the [data analysis](https://www.freqtrade.io/en/stable/strategy_analysis_example/#load-backtest-results-to-pandas-dataframe) backtesting section.

Also, you can use freqtrade in [webserver mode](https://www.freqtrade.io/en/stable/freq-ui/#backtesting) to visualize the backtest results in a web interface.
This mode also allows you to load existing backtest results, so you can analyze them without running the backtest again.

For this mode - `--notes ""` can be used to add notes to the backtest results, which will be shown in the web interface.
...
- Force-exits caused by `=-1` ROI entries use low as exit value, unless N falls on the candle open (e.g. `120: -1` for 1h candles)
...
This will save the results to `user_data/backtest_results/backtest-result-.json`, including results for both `Strategy001` and `Strategy002`.
...
Your next step is to learn [how to find optimal parameters with Hyperopt](https://www.freqtrade.io/en/stable/hyperopt/)
