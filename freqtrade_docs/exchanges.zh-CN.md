# 交易所说明（Exchange-specific Notes）

这份文档由 Freqtrade 官方页面离线保存后整理为便于“vibe coding 查阅使用”的 Markdown。

- 来源：https://www.freqtrade.io/en/stable/exchanges/
- 离线保存时间：Mon Jan 05 2026 11:41:50 GMT+0800 (中国标准时间)
...
| --- | --- | --- | --- |
| Binance | spot |  | limit |
| Binance | futures | isolated, cross | market, limit |
| Bingx | spot |  | market, limit |
| Bitmart | spot |  | ❌ (not supported) |
| Bitget | spot |  | market, limit |
| Bitget | futures | isolated | market, limit |
| Bybit | spot |  | ❌ (not supported) |
| Bybit | futures | isolated | market, limit |
| Gate.io | spot |  | limit |
| Gate.io | futures | isolated | limit |
| HTX | spot |  | limit |
| Hyperliquid | spot |  | ❌ (not supported) |
| Hyperliquid | futures | isolated, cross | limit |
| Kraken | spot |  | market, limit |
| OKX | spot |  | limit |
| OKX | futures | isolated | limit |
| Bitvavo | spot |  | ❌ (not supported) |
| Kucoin | spot |  | market, limit |
...
Binance supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force).
...
For Binance, it is suggested to add `"BNB/"` to your blacklist to avoid issues, unless you are willing to maintain enough extra `BNB` on the account or unless you're willing to disable using `BNB` for fees.
...
BingX supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force) with settings "GTC" (good till cancelled), "IOC" (immediate-or-cancel) and "PO" (Post only) settings.
...
Kraken supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force) with settings "GTC" (good till cancelled), "IOC" (immediate-or-cancel) and "PO" (Post only) settings.
...
Kucoin supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force) with settings "GTC" (good till cancelled), "FOK" (full-or-cancel) and "IOC" (immediate-or-cancel) settings.
...
For Kucoin, it is suggested to add `"KCS/"` to your blacklist to avoid issues, unless you are willing to maintain enough extra `KCS` on the account or unless you're willing to disable using `KCS` for fees.
...
Gate.io supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force) with settings "GTC" (good till cancelled), and "IOC" (immediate-or-cancel) settings.
...
Bybit supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force) with settings "GTC" (good till cancelled), "FOK" (full-or-cancel), "IOC" (immediate-or-cancel) and "PO" (Post only) settings.
...
Bitget supports [time_in_force](https://www.freqtrade.io/en/stable/configuration/#understand-order_time_in_force) with settings "GTC" (good till cancelled), "FOK" (full-or-cancel), "IOC" (immediate-or-cancel) and "PO" (Post only) settings.
...
- walletAddress in hex format: `0x` - Can be easily copied from your wallet - and should be your main wallet address, not your API Wallet Address.

- privateKey in hex format: `0x` - Use the key the API Wallet shows on creation.
...
Whether your exchange returns incomplete candles or not can be checked using [the helper script](https://www.freqtrade.io/en/stable/developer/#incomplete-candles) from the Contributor documentation.

Due to the danger of repainting, Freqtrade does not allow you to use this incomplete candle.

However, if it is based on the need for the latest price for your strategy - then this requirement can be acquired using the [data provider](https://www.freqtrade.io/en/stable/strategy-customization/#possible-options-for-dataprovider) from within the strategy.
