# 做空与杠杆（Trading with Leverage）

这份文档由 Freqtrade 官方页面离线保存后整理为便于“vibe coding 查阅使用”的 Markdown。

- 来源：https://www.freqtrade.io/en/stable/leverage/
- 离线保存时间：Mon Jan 05 2026 11:41:24 GMT+0800 (中国标准时间)
...
If you already have an existing strategy, please read the [strategy migration guide](https://www.freqtrade.io/en/stable/strategy_migration/#strategy-migration-between-v2-and-v3) to migrate your strategy from a freqtrade v2 strategy, to strategy of version 3 which can short and trade futures.
...
Please read [strategy customization](https://www.freqtrade.io/en/stable/strategy-customization/#entry-signal-rules) for instructions on how to set signals to enter and exit short trades.
...
Please read the [exchange specific notes](https://www.freqtrade.io/en/stable/exchanges/) for exchanges that support this mode and how they differ.
...
While you could configure one static leverage value - freqtrade offers you the flexibility to adjust this via [strategy leverage callback](https://www.freqtrade.io/en/stable/strategy-callbacks/#leverage-callback) - which allows you to use different leverages by pair, or based on some other factor benefitting your strategy result.
...
For futures data, exchanges commonly provide the futures candles, the marks, and the funding rates. However, it is common that whilst candles and marks might be available, the funding rates are not. This can affect backtesting timeranges, i.e. you may only be able to test recent timeranges and not earlier, experiencing the `No data found. Terminating.` error. To get around this, add the `futures_funding_rate` config option as listed in [configuration.md](https://www.freqtrade.io/en/stable/configuration/), and it is recommended that you set this to `0`, unless you know a given specific funding rate for your pair, exchange and timerange. Setting this to anything other than `0` can have drastic effects on your profit calculations within strategy, e.g. within the `custom_exit`, `custom_stoploss`, etc functions.
