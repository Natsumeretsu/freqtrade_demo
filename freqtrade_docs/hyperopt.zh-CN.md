...
- 来源：https://www.freqtrade.io/en/stable/hyperopt/
- 离线保存时间：Mon Jan 05 2026 11:39:01 GMT+0800 (中国标准时间)
...
To learn how to get data for the pairs and exchange you're interested in, head over to the [Data Downloading](https://www.freqtrade.io/en/stable/data-download/) section of the documentation.
...
Rarely you may also need to create a [nested class](https://www.freqtrade.io/en/stable/advanced-hyperopt/#overriding-pre-defined-spaces) named `HyperOpt` and implement
...
Hyperopt will then spawn into different processes (number of processors, or `-j `), and run backtesting over and over again, changing the parameters that are part of the `--spaces` defined.
...
- Guards are conditions like "never enter if ADX < 10", or never enter if current price is over EMA10.

- Triggers are ones that actually trigger entry in specific moment, like "enter when EMA5 crosses over EMA10" or "enter when close price touches lower Bollinger band".
...
A loss function must be specified via the `--hyperopt-loss ` argument (or optionally via the configuration under the `"hyperopt_loss"` key).
...
Creation of a custom loss function is covered in the [Advanced Hyperopt](https://www.freqtrade.io/en/stable/advanced-hyperopt/) part of the documentation.
...
Reading commands (`hyperopt-list`, `hyperopt-show`) can use `--hyperopt-filename ` to read and display older hyperopt results.
...
A sample for these methods can be found in the [overriding pre-defined spaces section](https://www.freqtrade.io/en/stable/advanced-hyperopt/#overriding-pre-defined-spaces).

Reduced search space

To limit the search space further, Decimals are limited to 3 decimal places (a precision of 0.001). This is usually sufficient, every value more precise than this will usually result in overfitted results. You can however [overriding pre-defined spaces](https://www.freqtrade.io/en/stable/advanced-hyperopt/#overriding-pre-defined-spaces) to change this to your needs.
...
Override the `stoploss_space()` method and define the desired range in it if you need stoploss values to vary in other range during hyperoptimization. A sample for this method can be found in the [overriding pre-defined spaces section](https://www.freqtrade.io/en/stable/advanced-hyperopt/#overriding-pre-defined-spaces).
...
Override the `trailing_space()` method and define the desired range in it if you need values of the trailing stop parameters to vary in other ranges during hyperoptimization. A sample for this method can be found in the [overriding pre-defined spaces section](https://www.freqtrade.io/en/stable/advanced-hyperopt/#overriding-pre-defined-spaces).
...
After you run Hyperopt for the desired amount of epochs, you can later list all results for analysis, select only best or profitable once, and show the details for any of the epochs previously evaluated. This can be done with the `hyperopt-list` and `hyperopt-show` sub-commands. The usage of these sub-commands is described in the [Utils](https://www.freqtrade.io/en/stable/utils/#list-hyperopt-results) chapter.
...
- You may have added parameters to hyperopt in `populate_indicators()` where they will be calculated only once **for all epochs**. If you are, for example, trying to optimise multiple SMA timeperiod values, the hyperoptable timeperiod parameter should be placed in `populate_entry_trend()` which is calculated every epoch. See [Optimizing an indicator parameter](#optimizing-an-indicator-parameter).
