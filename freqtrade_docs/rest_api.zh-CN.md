...
- 来源：https://www.freqtrade.io/en/stable/rest-api/
- 离线保存时间：Mon Jan 05 2026 11:38:27 GMT+0800 (中国标准时间)
...
FreqUI now has it's own dedicated [documentation section](https://www.freqtrade.io/en/stable/freq-ui/) - please refer to that section for all information regarding the FreqUI.
...
All endpoints in the below table need to be prefixed with the base URL of the API, e.g. `http://127.0.0.1:8080/api/v1/` - so the command becomes `http://127.0.0.1:8080/api/v1/`.
...
| /pair_candles | POST | Returns dataframe for a pair / timeframe combination while the bot is running, filtered by a provided list of columns to return. AlphaParams:- <column_list> (list[str]) |
| /pair_history | GET | Returns an analyzed dataframe for a given timerange, analyzed by a given strategy. Alpha |
| /pair_history | POST | Returns an analyzed dataframe for a given timerange, analyzed by a given strategy, filtered by a provided list of columns to return. AlphaParams:- <column_list> (list[str]) |
...
| /strategy/<strategy> | GET | Get specific Strategy content by strategy class name. AlphaParams:- <strategy> (str) |
...
This is also used to setup [Producer/Consumer mode](https://www.freqtrade.io/en/stable/producer-consumer/) in Freqtrade.
...
This will enable the Swagger UI at the `/docs` endpoint. By default, that's running at [http://localhost:8080/docs](http://localhost:8080/docs) - but it'll depend on your settings.
