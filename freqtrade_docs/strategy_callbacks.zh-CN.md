...
- 来源：https://www.freqtrade.io/en/stable/strategy-callbacks/
- 离线保存时间：Mon Jan 05 2026 11:36:08 GMT+0800 (中国标准时间)
...
While the main strategy functions (`populate_indicators()`, `populate_entry_trend()`, `populate_exit_trend()`) should be used in a vectorized way, and are only called [once during backtesting](https://www.freqtrade.io/en/stable/bot-basics/#backtesting-hyperopt-execution-logic), callbacks are called "whenever needed".
...
You can find the callback calling sequence in [bot-basics](https://www.freqtrade.io/en/stable/bot-basics/#bot-execution-logic)
...
- **`custom_stoploss()`** - Custom trailing stoploss, called for every open trade every iteration until a trade is closed. The value returned here is also used for [stoploss on exchange](https://www.freqtrade.io/en/stable/stoploss/#stop-loss-on-exchangefreqtrade).
...
You do not *have* to ensure that `min_stake
...
See [Dataframe access](https://www.freqtrade.io/en/stable/strategy-advanced/#dataframe-access) for more information about dataframe use in strategy callbacks.
...
Stoploss on exchange works similar to `trailing_stop`, and the stoploss on exchange is updated as configured in `stoploss_on_exchange_interval` ([More details about stoploss on exchange](https://www.freqtrade.io/en/stable/stoploss/#stop-loss-on-exchangefreqtrade)).

If you're on futures markets, please take note of the [stoploss and leverage](https://www.freqtrade.io/en/stable/stoploss/#stoploss-and-leverage) section, as the stoploss value returned from `custom_stoploss` is the risk for this trade - not the relative price movement.
...
For futures, we need to adjust the direction (up or down), as well as adjust for leverage, since the [`custom_stoploss`](#custom-stoploss) callback returns the ["risk for this trade"](https://www.freqtrade.io/en/stable/stoploss/#stoploss-and-leverage) - not the relative price movement.
...
By default, freqtrade use the orderbook to automatically set an order price ([Relevant documentation](https://www.freqtrade.io/en/stable/configuration/#prices-used-for-orders)), you also have the option to create custom order prices based on your strategy.
...
Incompatibility with `adjust_*_price()`
...
You can learn more about storing data on the [Storing custom trade data](https://www.freqtrade.io/en/stable/strategy-advanced/#storing-information-persistent) section.
