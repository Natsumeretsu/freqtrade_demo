...
- 来源：https://www.freqtrade.io/en/stable/strategy-customization/
- 离线保存时间：Mon Jan 05 2026 11:35:40 GMT+0800 (中国标准时间)
...
- the [Freqtrade strategy 101](https://www.freqtrade.io/en/stable/strategy-101/), which provides a quick start to strategy development

- the [Freqtrade bot basics](https://www.freqtrade.io/en/stable/bot-basics/), which provides overall info on how the bot operates
...
Check the [configuration documentation](https://www.freqtrade.io/en/stable/configuration/) about how to set the bot to dry or live mode.
...
[lookahead](https://www.freqtrade.io/en/stable/lookahead-analysis/) and [recursive](https://www.freqtrade.io/en/stable/recursive-analysis/) analysis.
...
You can use [recursive-analysis](https://www.freqtrade.io/en/stable/recursive-analysis/) to check and find the correct `startup_candle_count` to be used. When recursive analysis shows a variance of 0%, then you can be sure that you have enough startup candle data.
...
Instead, please check the [Storing information](https://www.freqtrade.io/en/stable/strategy-advanced/#storing-information-persistent) section.
...
When hyperopting, use of the hyperoptable parameter `.value` attribute is not supported. Please use the `.range` attribute. See [optimizing an indicator parameter](https://www.freqtrade.io/en/stable/hyperopt/#optimizing-an-indicator-parameter) for more information.
...
It's also not supported for FreqUI visualization in [webserver mode](https://www.freqtrade.io/en/stable/utils/#webserver-mode), as the configuration for webserver mode doesn't require a pairlist to be set.
...
Within [callbacks](https://www.freqtrade.io/en/stable/strategy-callbacks/), you'll get the full timerange up to the current (simulated) candle.

#### get_analyzed_dataframe(pair, timeframe)

This method is used by freqtrade internally to determine the last signal.
It can also be used in specific callbacks to get the signal that caused the action (see [Advanced Strategy Documentation](https://www.freqtrade.io/en/stable/strategy-advanced/) for more details on available callbacks).
...
Within [callbacks](https://www.freqtrade.io/en/stable/strategy-callbacks/), you'll get the wallet state corresponding to the actual simulated wallet at that point in the simulation process.
...
Locks can also be lifted manually, by calling `self.unlock_pair(pair)` or `self.unlock_reason()`, providing the reason the pair was unlocked.
`self.unlock_reason()` will unlock all pairs currently locked with the provided reason.
...
You should always use the two helper commands [lookahead-analysis](https://www.freqtrade.io/en/stable/lookahead-analysis/) and [recursive-analysis](https://www.freqtrade.io/en/stable/recursive-analysis/), which can each help you figure out problems with your strategy in different ways.
...
Your next step is to learn [how to use backtesting](https://www.freqtrade.io/en/stable/backtesting/).
