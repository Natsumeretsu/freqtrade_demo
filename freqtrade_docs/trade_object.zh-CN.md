# Trade 对象（Trade Object）

这份文档由 Freqtrade 官方页面离线保存后整理为便于“vibe coding 查阅使用”的 Markdown。

- 来源：https://www.freqtrade.io/en/stable/trade-object/
- 离线保存时间：Mon Jan 05 2026 11:42:36 GMT+0800 (中国标准时间)
...
It will be passed to the strategy in many [strategy callbacks](https://www.freqtrade.io/en/stable/strategy-callbacks/). The object passed to the strategy cannot be modified directly. Indirect modifications may occur based on callback results.

### Trade - Available attributes

The following attributes / properties are available for each individual trade - and can be used with `trade.` (e.g. `trade.pair`).
...
They can be used as `Trade.` - e.g. `open_trades = Trade.get_open_trade_count()`

Backtesting/hyperopt

Most methods will work in both backtesting / hyperopt and live/dry modes.
During backtesting, it's limited to usage in [strategy callbacks](https://www.freqtrade.io/en/stable/strategy-callbacks/). Usage in `populate_*()` methods is not supported and will result in wrong results.
...
| status | string | Status as defined by ccxt's order structure. Usually open, closed, expired, canceled or rejected |
