...
| 目录 | 用途 | 说明 |
|------|------|------|
| `configs/` | 配置模板 | 脱敏配置，可提交 |
| `configs/freqai/` | FreqAI 专用配置 | 分离管理 |
| `scripts/` | 辅助脚本 | 统一入口 `ft.ps1` |
| `experiments/` | 实验记录 | 命令/结论，不存产物 |
...
├── config-private.example.json  # 私钥配置示例
├── config_{strategy}.json       # 策略专用配置
...
```
❌ 禁止提交：
- config.json（含 API 密钥）
- *.env（环境变量）
- 任何含 exchange.key/secret 的文件
...
|-----------|------|
| `plot/*.html` | 图表可再生成 |
| `hyperopt_results/*.pkl` | 缓存数据 |
| `backtest_results/` 旧文件 | 保留最新即可 |
